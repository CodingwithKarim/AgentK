...
export function Menu({
  ...
  const toggleShared = () => setSharedContext(!sharedContext);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Existing toggles below */}
        <button
          role="menuitemcheckbox"
          aria-checked={sharedContext}
          onClick={() => setSharedContext(!sharedContext)}
        ...
        >
          ...
          <span className={`relative inline-flex h-4.5 w-8 items-center rounded-full ${sharedContext ? "bg-blue-600" : "bg-zinc-300"}`}>
            <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transform ${sharedContext ? "translate-x-4" : "translate-x-1"}`} />
          </span>
        </button>
